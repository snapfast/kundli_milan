export function generateMatchCardHtml(uid: string, name: string, age: number | undefined, category: string, score: number, description: string, kootaHtml: string, doshas: { name: string, description: string, isCancelled: boolean }[] = []): string {
            ...
            <div class="dosha-container" style="background-color: #fef2f2; border-left: 4px solid #ef4444; border-radius: 12px; padding: 1rem; margin: 1rem 0;">
                ...
                ${doshas.map(d => `
                    <div class="dosha-item ${d.isCancelled ? 'cancelled' : 'active'}" style="margin-bottom: 0.5rem; ${d.isCancelled ? 'opacity: 0.7;' : ''}">
                        <span class="dosha-name" style="font-weight: 700; font-size: var(--font-sm); color: #1e1d1a; ${d.isCancelled ? 'text-decoration: line-through;' : ''}">${d.name} ${d.isCancelled ? '(Cancelled)' : ''}</span>
                        <div class="dosha-description" style="font-size: var(--font-xs); color: #4a4947; line-height: 1.4; margin-top: 0.1rem;">${d.description}</div>
                    </div>
                `).join('')}
            </div>
        `;
    ...
    if (description) {
        ...
        let highlightColor = '#2c2b29';
        let highlightBg = '#f3f3f2';
        let highlightBorder = '#e4e4e2';
        ...
        descriptionHtml = `
            <div class="match-highlight" style="background: ${highlightBg}; border: 1.5px solid ${highlightBorder}; display: flex; gap: 0.75rem; padding: 1rem; border-radius: 14px; margin-bottom: 1rem; align-items: flex-start;">
                ...
                <div class="match-highlight-content" style="font-size: var(--font-sm); color: var(--bumble-black); line-height: 1.5;">
    ...
    return `
        <div class="match-header" style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1rem;">
            <div style="display: flex; flex-direction: column; gap: 0.2rem;">
                <span class="match-name" style="font-size: var(--font-xl); font-weight: 800; color: var(--bumble-black); letter-spacing: -0.02em;">${name}${age !== undefined ? `, ${age}` : ''}</span>
            ...
            <span class="match-category" style="background-color: var(--bumble-yellow); color: var(--bumble-black); padding: 0.35rem 0.75rem; border-radius: 9999px; font-size: var(--font-xs); font-weight: 800; text-transform: uppercase; border: 1.5px solid var(--bumble-black); box-shadow: 2px 2px 0 var(--bumble-black);">${category}</span>
        </div>
        <div class="match-score-container" style="background: var(--bumble-light-grey); padding: 0.75rem 1rem; border-radius: 12px; margin-bottom: 1rem;">
            <div class="match-score-header" style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 0.4rem;">
                <div class="match-score-value" style="font-size: var(--font-lg); font-weight: 800; color: var(--bumble-black);">${score} <small class="match-score-max" style="font-size: var(--font-xs); color: var(--bumble-text-light);">/ 36 Points</small></div>
